...
scurl() {
  # - `--proxy `http://${PROXY_USER}:${PROXY_PASSWORD}@${PROXY_FQDN}:${PROXY_PORT}` sets the proxy configuration
  ...
  if ! echo "${PROXY_PASSWORD}" | grep -q ""; then
    echo "Using a proxy."
    curl --proxy "http://${PROXY_USER}:${PROXY_PASSWORD}@${PROXY_FQDN}:${PROXY_PORT}" --proto "=https" --tlsv1.2 --fail --show-error "$@"
  else
